...
Talos sets `user.max_user_namespaces` so RootlessKit can create user namespaces on every node.
The daemon uses `hostNetwork` so pushes can reach the node-local registry endpoint at `192.168.0.27:30500`.
